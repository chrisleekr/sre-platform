...
export function IncidentEvidenceText({ text, label }: { text: string; label: string }) {
  ...
  const size = 6000;
  // Page by code point so a boundary never splits a surrogate pair into two replacement characters.
  const characters = useMemo(() => [...text], [text]);
  const count = Math.max(1, Math.ceil(characters.length / size));
  ...
  return (
    <div className="min-w-0">
      ...
      >
        {characters.slice(current * size, (current + 1) * size).join('') || 'No text returned.'}
      </pre>
      {count > 1 && (
