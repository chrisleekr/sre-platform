...
} from './stored-observation';
import { redactInput, scrubSecrets } from '@sre/agent-tools';
import type { SignalObservation } from '@sre/db';
...
export function alertmanagerMonitorKey(
  ...
  alert: Pick<NormalizedAlert, 'alertName' | 'fingerprint' | 'labels'> &
    Partial<Pick<NormalizedAlert, 'monitorIdentity' | 'provider'>>,
): string {
  ...
  return `${'provider' in alert && alert.provider ? alert.provider : 'alertmanager'}:${connectorId}:${hash(scope)}`;
}
...
export function signalObservation(
  ...
  const episodeKey = `${alert.provider ?? 'alertmanager'}:${connectorId}:${alert.fingerprint}:${alert.startsAt.toISOString()}`;
  return {
    ...
    materialHash,
    surface: alert.provider ?? 'alertmanager',
    channel: connectorId,
    externalMessageId: episodeKey,
    state: alert.status,
    clearProvenance: alert.status === 'resolved' ? 'provider' : undefined,
    ...
    eventKey: `${episodeKey}:${alert.status}:${materialHash}:${alert.endsAt?.toISOString() ?? ''}:generation:${lifecycleVersion ?? 'legacy'}`,
    eventAt: observedAt,
  };
}
